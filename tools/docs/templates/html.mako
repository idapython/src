<%
  ...
  def format_git_link(t, x):
    return None # function removed from htmp_helpers by clean.sh
...
<%def name="show_module(module)">
  ...
  <section>
    ...
    % for c in classes:
      ...
      mro = c.mro()
      # filter out "__plugins/loaders/procs__XXX" subclasses
      # ----------
      exclude = "__plugins__", "__loaders__", "__procs__"
      subclasses = []
      for subc in c.subclasses():
          for prefix in exclude:
              if subc.refname[:len(prefix)] == prefix:
                  break
          else:
              subclasses.append(subc)
      # ----------
...
<main>
  ...
  % else:
    <article id="content">
      <a style="float: right;" href="index.html">Module index</a>
      ${show_module(module)}
    </article>
    ${module_index(module)}
  % endif
